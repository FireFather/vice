...
int cnd_signal(cnd_t* cond)
{
	...
	const int haveWaiters = (cond->mWaitersCount > 0);
...
#if defined(_TTHREAD_WIN32_)
static int _cnd_timedwait_win32(cnd_t* cond, mtx_t* mtx, const DWORD timeout)
{
	...
	const DWORD result = WaitForMultipleObjects(2, cond->mEvents, false, timeout);
	...
	EnterCriticalSection(&cond->mWaitersCountLock);
	--cond->mWaitersCount;
	const int lastWaiter = (result == (WAIT_OBJECT_0 + _CONDITION_EVENT_ALL)) &&
		(cond->mWaitersCount == 0);
	LeaveCriticalSection(&cond->mWaitersCountLock);
...
int cnd_timedwait(cnd_t* cond, mtx_t* mtx, const struct timespec* ts)
{
	...
	{
		const unsigned long long nowInMilliseconds = now.tv_sec * 1000 + now.tv_nsec / 1000000;
		const unsigned long long tsInMilliseconds = ts->tv_sec * 1000 + ts->tv_nsec / 1000000;
		const DWORD delta = (tsInMilliseconds > nowInMilliseconds) ? (DWORD)(tsInMilliseconds - nowInMilliseconds) : 0;
...
static void _tinycthread_tss_cleanup(void)
{
	...
	for (int iteration = 0; iteration < TSS_DTOR_ITERATIONS && again > 0; iteration++)
	{
		again = 0;
...
static void NTAPI _tinycthread_tss_callback(const PVOID h, const DWORD dwReason, const PVOID pv)
{
	(void)h;
	(void)pv;
...
#endif
{
	...
	_thread_start_info* ti = aArg;
	const thrd_start_t fun = ti->mFunction;
	void* arg = ti->mArg;
...
int thrd_create(thrd_t* thr, const thrd_start_t func, void* arg)
{
	/* Fill out the thread startup information (passed to the thread wrapper,
	   which will eventually free it) */
	_thread_start_info* ti = malloc(sizeof(_thread_start_info));
	if (ti == NULL)
	{
		return thrd_nomem;
	}
	...
	/* Create the thread */
#if defined(_TTHREAD_WIN32_)
	*thr = CreateThread(NULL, 0, _thrd_wrapper_function, ti, 0, NULL);
#elif defined(_TTHREAD_POSIX_)
...
int thrd_sleep(const struct timespec* duration, struct timespec* remaining)
{
  ...
  if (res == 0) {
	...
	const DWORD t = SleepEx(duration->tv_sec * 1000 +
...
{
#if defined(_TTHREAD_WIN32_)
	struct TinyCThreadTSSData* data = TlsGetValue(key);
...
{
#if defined(_TTHREAD_WIN32_)
	const struct TinyCThreadTSSData* data = TlsGetValue(key);
	if (data == NULL)
	{
		return NULL;
	}
